...
impl Client {
    ...
    pub(crate) async fn send_message_impl(
        ...
        let stanza_to_send: wacore_binary::Node = if peer && !to.is_group() {
            ...
            // Backport of upstream fix #634 ("self-DM stops working"): for a
            // self-DM, merging the PN-keyed own-device list on top of the
            // LID-keyed recipient list addresses every physical device twice
            // (once per namespace). Both copies resolve to the same Signal
            // session, the parallel encrypt fan-out races itself over the
            // destructive cache checkout ("session with X not found"), and
            // the server rejects the duplicated participant list with
            // ack error="400".
            let is_self_dm =
