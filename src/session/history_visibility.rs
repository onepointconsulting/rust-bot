...
use crate::session::{automation_turns::{has_message_value, is_automation_history_message}, keys::HIDDEN_HISTORY_KEY};
...
mod tests {
    ...
    fn exact_true_marker_is_hidden() {
        assert!(has_hidden_history_marker(&message_with_marker(Some(json!(true)))));
    }

    #[test]
    fn object_marker_is_hidden() {
        assert!(has_hidden_history_marker(&message_with_marker(Some(json!({})))));
    ...
    fn other_marker_values_are_not_hidden() {
        assert!(!has_hidden_history_marker(&message_with_marker(Some(json!(false)))));
        assert!(!has_hidden_history_marker(&message_with_marker(Some(json!(null)))));
        assert!(!has_hidden_history_marker(&message_with_marker(Some(json!(1)))));
        assert!(!has_hidden_history_marker(&message_with_marker(Some(json!("true")))));
        assert!(!has_hidden_history_marker(&message_with_marker(Some(json!([])))));
    }
}
