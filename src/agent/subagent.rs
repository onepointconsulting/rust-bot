...
use crate::{
    agent::{
        context::ContextBuilder, hook::{AgentHook, AgentHookContext}, runner::{AgentRunResult, AgentRunner}, skills::SkillsLoader
    },
...
impl SubagentManager {
    ...
    fn format_partial_progress(result: AgentRunResult) -> String {
        let completed = result.tool_events.iter().filter(|e| e.get("status").unwrap_or(&"".to_string()) == "ok").collect::<Vec<_>>();
        let failure = result.tool_events.iter().rev().find(|e| e.get("status").unwrap_or(&"".to_string()) == "error");
        let mut lines = Vec::new();
        if !completed.is_empty() {
            lines.push("Completed steps:".to_string());
            let start = completed.len().saturating_sub(3);
            for event in completed[start..].iter() {
                lines.push(format!("- {}: {}", event.get("name").unwrap_or(&"".to_string()), event.get("detail").unwrap_or(&"".to_string())));
            }
        }
        if let Some(failure) = failure {
            ...
            lines.push(format!("- {}: {}", failure.get("name").unwrap_or(&"".to_string()), failure.get("detail").unwrap_or(&"".to_string())));
        }
...
mod tests {
    ...
    fn format_partial_progress_shows_last_three_completed_steps_in_order() {
        ...
        assert!(step2 < step3 && step3 < step4, "steps should stay chronological");
    }
